...
**核心原则：面向用户的内容用中文，机器标识用英文。**

- **中文**：日志输出、错误消息、帮助文本、CLI 输出、代码注释（逻辑说明/设计决策）、文档正文
- **英文**：函数名、变量名、`Usage:` 行（函数签名描述）、Git ref、URL

> `Usage:` 行的作用是描述函数签名（参数名和类型），与函数名保持一致，因此使用英文。<br>函数行为和返回值的描述注释（如 "Returns the file size in bytes"）同理——可视为 Usage: 行的延伸，使用英文。
