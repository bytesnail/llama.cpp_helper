...
> 本项目是针对 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的自动化构建与管理 **Bash 脚本工具集**（5 个脚本，1824 LOC），面向双路 RTX 2080 Ti (NVLink) 工作站调优。**不含 C/C++ 代码**——通过调用 CMake/Ninja/git 等外部工具构建位于相邻目录 `../llama.cpp` 的上游源码。
