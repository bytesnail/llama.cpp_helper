...
- `_LLAMA_RUN_SILENT_TMP`：`llama_run_silent` 登记当前捕获临时文件的路径，`llama_safe_exit` 统一退出漏斗据此清理——信号路径（trap → `llama_safe_exit` → exit）不会执行 `llama_run_silent` 自身的 `rm -f`，缺该登记会把整份构建日志遗留在 `$TMPDIR`。
...
- **命令包装**：`llama_run_silent <rc_var> <cmd...>` 临时禁用 `set -e` 运行命令并捕获输出——**恒返回 0**，退出码经 `printf -v` 写入 `<rc_var>`（必写，`set -u` 下读取安全）；调用点先 `local` 声明该变量（动态作用域下 `printf -v` 才会写入局部变量），再读取它决定失败响应（die/回滚）。误用（缺/非法变量名、保留前缀 `_lrs_`、缺命令）返回 2 大声失败——与被包装命令的失败是两类。适用边界：仅短命令（秒级，如 CMake 配置）——长时命令（ninja 编译、build.sh 子进程）必须直跑流式输出（`cmd || rc=$?` 捕获退出码），静默包装会使数十分钟构建期间控制台零输出（曾实证 update.sh 的成功运行不含 build.sh 任何一行输出）
